...
> How we moved from template-based extensions to agent skills, ran 92 evaluations to measure them, and learned what makes a skill worth keeping
> 

Recently, we started migrating Scaffold-ETH 2's template-based extension system with markdown skill files. To measure the capability uplift, we used claude codes A/B eval pipeline and ran 92 evaluations across. Here is how we isolated true capability from baseline model knowledge, mitigated context pollution, and discovered why self-evaluating agents artificially inflated our success metrics by 60 percentage points.
...
**With skill (Variant A):** a natural task description plus instructions to read the [SKILL.md](http://SKILL.md) file. "Add x402 payment-gated API routes to my SE-2 dApp" plus the skill.
...
**Teaching to the test.** When the executor sees an assertion like "Uses CAIP-2 network format (eip155:84532)" before writing code, it just does that. The assertions become a requirements document, not evaluation criteria. It doesn't matter whether Claude "knows" about CAIP-2 from training. Without seeing the assertion, it defaults to whatever API surface it remembers. With the assertion, it has a cheat sheet.
...
We fixed everything. Split execution and grading into two separate phases. The executor gets only the task prompt, no assertions, no hints. A separate grader reads the output files against the assertions. We stripped the skill index from [AGENTS.md](http://AGENTS.md) for baseline runs. Bumped to 5 runs per configuration. 40 total runs, 80 agent invocations.
...
### Why the failures are systematic, not random
...
**API versioning.** For rapidly evolving libraries, the model reaches for API shapes from training, which means the wrong version. x402 restructured everything between v1 and v2. Ponder changed its package name, its schema API, and its handler format in v0.7. The model confidently writes code against APIs that no longer exist. This shows up in 5 of 10 x402 assertions and 5 of 10 ponder assertions.

**SE-2 integration bridges.** These are conventions we designed for the monorepo: reading `deployedContracts.ts` for contract addresses, using `scaffoldConfig` for network info, workspace naming (`@se-2/ponder`), root proxy scripts, environment variable conventions. The model can't know these because we made them up. EIP-5792 is interesting here because Claude knows the standard well technically (6 of 10 assertions pass) but misses the UX layer: fallback behavior for wallets that don't support batching, status display with `useShowCallsStatus`, defensive disabling of the batch button.
...
We trimmed them down aggressively. Four skills with delta at or below 5pp: removed entirely. EIP-712 and SIWE: cut everything the model already knows, kept only the discriminating content. 2,123 lines down to 365. An 83% reduction.

![image.png](image%202.png)

## Trimming tier 1 skills to prevent context pollution

The eval data didn't just help with tier 2+3. We went back through every tier 1 skill and mapped each section to a specific assertion. If the content was non-discriminating (the model gets it right without the skill), we cut it.
...
When we ran iteration 4, the ERC-20 skill showed a +5pp delta and we dismissed it as noise. Then one of our teammates who'd been writing Solidity contracts for years tested it manually and the code with the skill was noticeably better. He pointed to specific things: named imports (`import {ERC20}` instead of `import "..."`), `_update` override (required for OZ v5 compilation), `formatUnits` with dynamic decimals from `viem`.
...
**Separate execution from grading** - Self-grading inflated our scores by 60pp. The executor gets the task prompt and nothing else, a separate grader checks the output.
